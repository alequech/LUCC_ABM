...
to view_bmmr
  ;gis:apply-raster MBBR-data MBBR-patch  ;patches in the land-use-map have a specific land-use now
  ask patches [
   if MBBR-patch = 1 [ set pcolor 64 ] ; Green = Core zone
   if MBBR-patch = 2 [ set pcolor 14 ] ; Dark red = Buffer zone
  ]
end
to go
...
to a_avocado
  ask patches [
  if ppt-patch >= 950 and ppt-patch <= 1100 and tem-patch >= 15 and tem-patch <= 24 and MBBR-patch = 2 and luc-patch != 6 and luc-patch != 7
    [set w_attitude 1]
  if ppt-patch < 950 and ppt-patch > 1100 and tem-patch < 15 and tem-patch > 24 and MBBR-patch = 2
    [set w_attitude 2]; 1)optimal climate attitude 2)
  ]
end

to view_attitude
  ask  patches [
   if w_attitude = 1 [ set pcolor white ]
   if w_attitude = 2 [ set pcolor orange ]
  ]
end
